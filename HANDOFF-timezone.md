...
- main = `a285783`（PR #23 マージ済み）。798 tests / カバレッジ lines 97.4%・branches 90.72%・functions 96.01%（閾値 90 / 85 / 90）
- **Version Packages PR #21 が open**（0.1.4 → 0.1.5）。マージすると npm に publish される。
  今回の変更は破壊的なので #21 とは切り離し、**#21 を先に出してから**着手するのが安全（後述）
...
Version Packages の #21（0.1.5）が open なので、**#21 を先にマージして publish してから**
この作業のブランチを切るのが安全。混ぜると 1 リリースに patch と破壊的変更が同居する。
...
- **マージはしない。** Version Packages の #21（0.1.5）が open なので、
  破壊的変更を patch リリースに混ぜないよう、#21 を先に出してからマージするのが安全。
  その旨を PR 本文に一言書いておくとレビューする側が判断できる
