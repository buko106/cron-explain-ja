...
- 時の範囲が 2 つ以上ある式は、最初の範囲しか読めない。
  `0 0-2,20-23 * * *` →「毎日午前0時から午前2時までと午後8時から午後11時まで毎時0分」→
  `0 0-2 * * *`。`collect` が時刻を `TimeValue[]` の平坦な並びで持ち、範囲かどうかを
  `timeMode` の 1 つの旗で表しているため、`times[0]`-`times[1]` の 1 組しか作れない。
  §2.3 の値の並び（`ValueList`）と同じように、時刻も範囲を含む並びとして持ち直す必要がある。
  時間帯語（「朝」「夕方」）が `standaloneWords` として後ろに回される点も一緒に見直すこと
