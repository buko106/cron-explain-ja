...
- parse が「毎月1日から3日ごと」のように日付と日の間隔が並ぶ入力で、間隔を黙って捨てて
  ...
  note と減点が要る
- 「AからBまでNごと」を時の範囲の中に置くと「まで」が重なる。
  `5/15 9-17 * * *` →「午前9時から午後5時まで5分から59分まで15分ごと」。
  `N/M` は `N-max/M` に正規化されるため、起点が下限でない刻みすべてで起きる。
  分の節を自己完結した「AからBまで」で組み立てていることが原因で、
  畳み込みではなく合成の側を直す必要がある
- `test/fixtures/explain-real.jsonl` の 165 式（拡張構文・秒付きを除く）を
  explain → parse で往復させると 16 件で意味が変わる。いずれも parse 側の欠陥で、
  explain の出力は正しい。内訳は次の 4 系統。
  1. 分・時のリストが最初の要素しか読まれない
     （「毎時9分と39分」→ `9 * * * *`、「毎日午前0時と正午」→ `0 0 * * *`）
  2. 「AからBまで」がリストになる
     （「28日から31日まで」→ `28,31`、「火曜日から土曜日まで」→ `2,6`）
  3. 「Nか月ごと」が無視される（「3か月ごとの1日」→ `0 0 1 * *`）
  4. 「AからBまでN分ごと」の起点が落ちる
     （「1分から59分まで2分ごと」→ `*/2`。奇数分が偶数分になり意味が反転する）

  往復テスト（§3.3）にこのコーパスを繋ぐのは、上を直してからにする