...
| 毎日夜9時 | `0 21 * * *` | 1.0 | 「夜」の指す時間帯に入っている |
| 毎日夜6時 | `0 18 * * *` | 0.9 | 近い読みに寄せ、離れたぶんを note |
| 毎晩6時 | `0 18 * * *` | 0.9 | 「毎晩」＝「毎日」＋「夜」 |
...
const RULES: Array<[RegExp, (m: RegExpMatchArray) => Token]> = [
  ...
  [/^(早朝|朝|昼|正午|夕方|夜中|深夜|夜|晩)/, m => ({ type: 'TIME_OF_DAY', word: m[1] })],
  [/^(午前|午後)/,                  m => ({ type: 'AMPM', value: m[1] })],
  [/^毎(分|時|日|週|月|年)/,        m => ({ type: 'FREQ', value: FREQ['毎'+m[1]] })],
  [/^毎(?=早朝|朝|正午|昼|夕方|夜中|深夜|夜|晩)/, () => ({ type: 'FREQ', value: 'day' })],
...
「毎晩」「毎朝」は「毎日」＋時間帯語なので、「毎」だけを頻度語として切り出し、続く語は
時間帯語の規則に任せる。時間帯語ごとに `毎〜` を辞書へ足さずに済む。
...
- `TIME` → hour/minute。直前の `AMPM`/`TIME_OF_DAY` で補正（「午後3時」→15、「夜9時」→21）。
  ...
  **語の range に近い方**を採る（「夜6時」→18、「昼2時」→14）。距離は時計の巡回で測るので、
  12 時のもう一方の読みは 0 時になる（「午前12時」「夜12時」→0）。
  13 時以降は 24 時間制としか読めないのでずらさない（「朝22時」→22）
- 採った hour が range 外なら note、range からの距離 1 時間につき -0.1（上限 -0.3）。
  range の外に出た瞬間に読みが裏返る（「夜6時」→午前6時）のは行き過ぎなので、
  読みは近い方に寄せ、寄せたぶんの不確かさは confidence で伝える
...
- 「AからBまで」の終端に時間帯語が書かれていなければ、始点に掛かっていた語で読む
  （「夜6時から9時まで」→ `18-21`）。終端を字面どおり 9 時と読むと `18-9` と日をまたぐ。
  引き継いだ語のずれは note にも減点にもしない（利用者が書いた語ではないため）
...
- `毎日` → dom/dow が any を期待（違えば note、-0.1）。
  「毎晩」「毎朝」は tokenize が `毎日` + 時間帯語に分けるので、ここでは区別しない
...
- 時間帯語と並んだ時は、range に入っているかどうかではなく **range に近い方の読み**を採る。
  以前は range に入る読みが無ければ書かれたまま採っていたので、「夜」（19-23）の 1 時間手前の
  「夜6時」が午前 6 時になっていた。時間帯語の境界は本来ぼやけているのに、そこで読みが
  裏返るのは利用者から見て説明が付かない。近い方に寄せたうえで、range から離れたぶんを
  1 時間につき -0.1（上限 -0.3）して note に書く。「朝22時」のような 13 時以降の数字は
  24 時間制としか読めないので、寄せる先が無く note と減点だけになる
- 読みを寄せる規則は「AからBまで」の終端にも及ぶ。「夜6時から9時まで」の終端に語は無いが、
  始点の「夜」で読まないと `18-9` と日をまたぐ範囲になる。始点の語を引き継いで `18-21` とし、
  引き継いだ語のずれは note にも減点にもしない（書いたのは利用者ではないため）
- 「毎晩」は「毎日」＋「夜」として読む。「毎」を頻度語として切り出す規則にしたので、
  「毎朝」「毎夜」も同じ形で読める。「晩」は「夜」と同じ既定値・range を持つ別の語にした
  （「夜中」と「深夜」も同じ扱い）。`timeOfDay` で上書きするときは語ごとに指定する
