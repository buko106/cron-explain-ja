...
- 「N時台」と間隔が並ぶ入力（「午前9時台の10分ごと」）を、間隔と単独時刻の併用として
  ambiguity にしている。「台」はその時の中という意味なので本当は曖昧ではないが、
  式は正しく出る（`*/10 9 * * *`）ため confidence が 0.8 に留まるだけの問題として残した。
  「台」を時の範囲を表すトークンとして読めば消せる
